...
Για κάθε αίτημα εμφανίζει, με τις ΠΡΑΓΜΑΤΙΚΕΣ τιμές που παράγει ο θωρακισμένος
βοηθός (όχι placeholders):

  * την ταξινόμηση του αιτήματος (τύπος εργασίας / ύποπτο για επίθεση),
  * έναν δείκτη κινδύνου εισόδου και έναν εξόδου (πραγματικές βαθμολογίες από το defense_info),
  * ένα ίχνος άμυνας που δείχνει ποιο από τα τρία επίπεδα ενεργοποιήθηκε και γιατί,
  * την τελική απόφαση (έγκριση / φιλτράρισμα / μπλοκάρισμα) και πού έγινε το μπλοκ,
  * τον πραγματικά μετρημένο χρόνο επεξεργασίας.


Χρήση:
    python3 dashboard.py --demo      # τρία αντιπροσωπευτικά αιτήματα
    python3 dashboard.py             # διαδραστικά
"""
...
def risk_meter(score: float, label: str) -> str:
    # Φτιάχνει μια οπτική μπάρα κινδύνου (γεμάτη/άδεια) και μια ετικέτα LOW/MEDIUM/HIGH.
...
def defense_trace(di: Dict[str, Any]) -> str:
    # Χτίζει το ίχνος: για κάθε επίπεδο δείχνει αν ήταν ανενεργό, καθαρό, ή ενεργοποιήθηκε
    # (και με ποια μοτίβα). Διαβάζει τα πάντα από το defense_info του θωρακισμένου βοηθού.
    lines = ["  Defense trace:"]
...
def render(prompt: str, document: str, response: str, task_type: str,
    ...
    # Συναρμολογεί όλο τον πίνακα ελέγχου ως κείμενο: επικεφαλίδα, ταξινόμηση,
    # δείκτες κινδύνου, ίχνος άμυνας, τελική απόφαση, χρόνο και απάντηση.
    decision = di.get("decision", "approved")
    blocked_at = di.get("blocked_at")
    # Θεωρείται ύποπτο αν ο κίνδυνος εισόδου είναι υψηλός ή αν μπλοκαρίστηκε/φιλτραρίστηκε.
...
def analyse(assistant: DefendedAssistant, prompt: str, document: str = "") -> str:
    # Μετράει τον πραγματικό χρόνο γύρω από την κλήση του βοηθού και επιστρέφει τον πίνακα.
...
def demo() -> None:
    # Τρία αντιπροσωπευτικά αιτήματα: ένα καλόπιστο, μία ωμή επίθεση, μία έμμεση ένεση.
    a = DefendedAssistant()
